...
from c2nl.tokenizers.simple_tokenizer import SimpleTokenizer
...
def tokenize(input):
    pattern = r"(?<!\\)(['\"])(.*?)(?<!\\)\1"
    text = re.sub(pattern, r"STRING", input)

    tokenizer = CodeTokenizer()
    tokens = []
    for token in tokenizer.tokenize(text).words():
        tokenizer = SimpleTokenizer()
        tokens += tokenizer.tokenize(token).words()

    tokenized = ' '.join(tokens)

    for word in tokenized.split():
        if word.isdigit():
            tokenized = tokenized.replace(word, 'NUM')
        elif word in ['true', 'false', 'True', 'False']:
            tokenized = tokenized.replace(word, '_BOOL')
...
def process_examples_to_test(lang_id, source, max_src_len, code_tag_type):
...
def load_data_to_test(args, dataset_name='java'):
    return [process_examples_to_test(LANG_ID_MAP[DATA_LANG_MAP[dataset_name]],
                                    args.source_code_request_body,
                                    args.max_src_len,
                                    args.code_tag_type)]
